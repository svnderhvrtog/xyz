...
const wonText1 = document.getElementById('won-text1');
const wonText2 = document.getElementById('won-text2');
...
// Audio
const victorySound = new Audio('../audio/poke/victory.mp3');
...
function attackOneBvsSFunct() {
   let quickAttack = Math.floor( Math.random() * 10 ) + 5;
    if (attackCounterOneNumBul == 0){
        logTextOne.innerText += "Attack is not available anymore! \n";
        return
    };
    quickSound.play();
...
function challengerBvsSAttackOneFunct() {
    ...
    if (playerStartHealthBulbasaur < 0) {
        wonText1.innerText = 'YOU LOST';
        wonText2.innerText = 'Keep on training!';
        challengerBvsSWin();
    }
}

function challengerBvsSWin() {
    battleOne.style.display = "none";
    youWon.style.display = "block";
    setTimeout(() => {
        wonText1.innerText = 'YOU WON';
        wonText2.innerText = 'Well done trainer!';
...
function attackOneBvsVFunct() {
   let quickAttack = Math.floor( Math.random() * 10 ) + 5;
    if (attackCounterOneNumBulSec == 0){
        logTextTwo.innerText += "Attack is not available anymore! \n";
        return
    };
    quickSound.play();
...
function attackOnePvsSFunct() {
   let tackle = Math.floor( Math.random() * 20 ) + 4;
    if (attackCounterOneNum == 0){
        logTextThree.innerText += "Attack is not available anymore! \n";
        return;
    };
    tackleSound.play();
...
function attackOnePvsVFunct() {
   let tackle = Math.floor( Math.random() * 20 ) + 4;
    if (attackCounterOneNumSec == 0){
        logTextFour.innerText += "Attack is not available anymore! \n";
        return
    };
    tackleSound.play();
